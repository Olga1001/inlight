...
function render() {
  // clear the canvas
  ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height); // choose the dot color

  ctx.fillStyle = 'rgba(249,249,249, 0.2)'; // stage2.fillStyle = 'rgba(136,145,163, 0.2)';
  // for each dot
...
function getV(dot) {
  ...
  var d = getDistance(dot, mouse); // reverse the distance, so that the number is bigger when the mouse is closer.
